...
api_pulls <- function(year, variablecode, name, statefips, countyfips) {
  ...
  name_year <- str_c(name, year)
  
  ggsave(filename = str_c("image/", name_year, ".png"), device = "png", plot = m)
}
...
name <- c("Median Household Income", "Percent with Bachelors Degree", 
          "Percent Below Povery Rate", "Percent with Bachelors Degree", 
          "Median Household Income", "Percent Below Poverty Rate")

state <- c(17, 12, 11, 40, 10, 08)

county <- c(031, 03, 01, 09, 05, 05)
